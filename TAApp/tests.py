...
class TestModels(TestCase):
    def setUp(self):
        ...
        course = Course.objects.create(name="course", description="A course", project_manager=administrator,
                                       instructor=instructor, instructorTA=instructorTA)
        lab = Lab.objects.create(name='lab', description='lab for course', project_manager=instructor,
                                 labTA=instructorTA, labForCourse=course)
...
class TestAdminHomeViews(BaseTest):
    ...
    def test_admin_page_load_TA_list(self):
        ...
        self.client.post(self.register_url,
                         data={'name': testAdmin.name, 'password': testAdmin.password, 'job': testAdmin.job},
                         format='text/html')
    ...
    def test_admin_page_load_course_list(self):
        ...
        testTA = TA.objects.create(name="TA", project_manager=testInstructor)
        testTA.save()
        testCourse = Course.objects.create(name="Course", description="Course Description", project_manager=testAdmin,
                                           instructor=testInstructor, instructorTA=testTA)
        testCourse.save()
        self.client.post(self.register_url,
                         data={'name': testAdmin.name, 'password': testAdmin.password, 'job': testAdmin.job},
                         format='text/html')
...
class TestCourseViews(BaseTest):
    ...
    def test_admin_can_create_course(self):
        ...
        self.client.post(self.login_url, data={'name': testAdmin.name, 'password': testAdmin.password},
                         format='text/html')
        response = self.client.post(self.courses,
                                    data={'name': testCourse.name, 'instructorchoice': testInstructor.name,
                                          'tachoice': testTA.name, 'description': testCourse.description},
                                    format='text/html')
        self.assertEqual(response.status_code, 302)
    ...
    def test_instructor_can_create_course(self):
        ...
        testCourse.save()
        self.client.post(self.register_url,
                         data={'name': testInstructor.name, 'password': "Instructor", 'job': testInstructor.role},
                         format='text/html')
        response = self.client.post(self.login_url, data={'name': testInstructor.name, 'password': "Instructor"},
                                    format='text/html')
        self.assertEqual(response.status_code, 302)
        createCourseResponse = self.client.post(self.courses,
                                                data={'name': testCourse.name, 'instructorchoice': otherInstructor.name,
                                                      'tachoice': testTA.name, 'description': testCourse.description})
...
class TestEditCourses(BaseTest):
    ...
    def test_admin_correct_courses(self):
        ...
        testCourse.save()
        self.client.post(self.register_url,
                         data={'name': testAdmin.name, 'password': testAdmin.password, 'job': "Admin"})
        self.client.post(self.login_url, data={'name': testAdmin.name, 'password': testAdmin.password})
        response = self.client.get(self.edit_courses_url)
...
class TestCreateTA(BaseTest):
    ...
    def test_admin_create_TA(self):
        ...
        self.client.post(self.login_url, data={'name': testAdmin.name, 'password': testAdmin.password},
                         format="text/html")
        response = self.client.post(self.create_TA_url,
                                    data={'name': "testTA", 'instructorchoice': testInstructor, 'password': "testTA"},
                                    format="text/html")
        self.assertEqual(response.status_code, 302)
    ...
    def test_instructor_create_TA(self):
        ...
        self.client.post(self.register_url,
                         data={'name': testInstructor.name, 'password': "password", 'job': "Instructor"},
                         format="text/html")
        self.client.post(self.login_url, data={'name': testInstructor.name, 'password': "password"}, format="text/hmtl")
        response = self.client.post(self.create_TA_url,
                                    data={'name': "testTA", 'instructorchoice': anotherInstructor.name,
                                          'password': "password"}, format="text/html")
        self.assertEqual(response.status_code, 302)
    ...
    def test_user_did_not_fill_in_the_box(self):
        ...
        self.client.post(self.login_url, data={'name': testAdmin.name, 'password': testAdmin.password},
                         format="text/html")
        self.client.post(self.create_TA_url,
                         data={'name': "", 'instructorchoice': testInstructor.name, 'password': "testTA"},
                         format="text/html")
        response = self.client.get(self.create_TA_url)
    ...
    def test_TA_is_already_added(self):
        ...
        self.client.post(self.login_url, data={'name': testAdmin.name, 'password': testAdmin.password},
                         format="text/html")
        self.client.post(self.create_TA_url,
                         data={'name': testTA.name, 'instructorchoice': testInstructor.name, 'password': "testTA"},
                         format="text/html")
        response = self.client.get(self.create_TA_url)
        messages = list(response.context['messages'])
        self.assertEqual(len(messages), 1)
...
class TestTAHomeViews(BaseTest):
    ...
    def test_page_loads_labs(self):
        ...
        testCourse = Course.objects.create(name="Course", description="Course Description", project_manager=testAdmin,
                                           instructor=testInstructor, instructorTA=testTA)
        testCourse.save()
        testLab = Lab.objects.create(name="Lab", description="Description for Lab", project_manager=testInstructor,
                                     labTA=testTA, labForCourse=testCourse)
        testLab.save()
        self.client.post(self.register_url, data={'name': testTA.name, 'password': "Password", 'job': "TA"},
                         format="text/html")
        response = self.client.post(self.login_url, data={'name': testTA.name, 'password': "Password"},
                                    format="text/html")
        self.assertEqual(response.status_code, 302)
...
class TestCreateInstructor(BaseTest):
    ...
    def test_admin_add_instructor_success(self):
        ...
        self.client.post(self.login_url, data={'name': testAdmin.name, 'password': testAdmin.password},
                         format="text/html")
        response = self.client.post(self.create_instructor_url,
                                    data={'name': testAdmin.name, 'instructor': "Instructor2",
    ...
    def test_instructor_add_instructor_success(self):
        ...
        self.client.post(self.login_url, self.instructor, format="text/html")
        response = self.client.post(self.create_instructor_url,
                                    data={'name': self.instructor, 'instructor': "TestInstructor",
    ...
    def test_fields_were_not_filled_in(self):
        self.client.post(self.register_url, self.instructor, format="text/html")
        self.client.post(self.login_url, self.instructor, format="text/html")
        self.client.post(self.create_instructor_url,
                         data={'name': self.instructor, 'instructor': "", 'password': "Password"})
    ...
    def test_instructor_already_exists(self):
        ...
        self.client.post(self.login_url, data={'name': testAdmin.name, 'password': testAdmin.password},
                         format="text/html")
        self.client.post(self.create_instructor_url,
                         data={'name': testAdmin.name, 'instructor': "testInstructor", 'password': "Password"})
        response = self.client.get(self.create_instructor_url)
        messages = list(response.context['messages'])
        self.assertEqual(len(messages), 1)
...
class TestAssignTAs(BaseTest):
    ...
    def test_admin_can_assign_TA(self):
        ...
        self.client.post(self.login_url, data={'name': testAdmin.name, 'password': testAdmin.password},
                         format="text/html")
        self.client.post(self.assign_TA,
                         data={'coursechoice': testCourse.name, 'tachoice': testTA.name, 'lab': testLab.name})
        response = self.client.get(self.assign_TA)
        messages = list(response.context['messages'])
        self.assertEqual(len(messages), 1)
...
class TestAdminLogin(TestCase):
    ...
    def test_existing_user(self):
        ...
        self.assertFalse(response.context['user'], "No such user.",
...
class TestAddCourse(TestCase):
    ...
    def setUp(self):
        ...
        for i in self.courses.keys():
            ...
            for j in self.courses[i]:
                Courses(name="test" + j, description="example" + j, project_manager=admin, instructor=instructor,
                        instructorTA=technicalAssistant).save()
    ...
    def test_course_added(self):
        ...
        response = self.client.post("/courses/",
                                    {"name": "new course", "instructor": "instructor", "instructorTA": "TA",
                                     "description": "example"}, follow=True)
        self.assertListEqual(["math", "science", "new course"], response.context["courses"],
                             "The course wasn't added successfully")

    def test_bad_name(self):
        response = self.client.post("/courses/", {"name": "", "instructor": "instructor", "instructorTA": "TA",
                                                  "description": "example"}, follow=True)
        self.assertFalse(response.context["name"], "", "The course was added when it wasn't supposed to be")

    def test_bad_description(self):
        response = self.client.post("/courses/", {"name": "test", "instructor": "instructor", "instructorTA": "TA",
    ...
    def test_bad_instructor(self):
        response = self.client.post("/courses/", {"name": "test", "instructor": "Superman", "instructorTA": "TA",
                                                  "description": "example"}, follow=True)
        self.assertFalse(response.context["instructor"], "Superman",
                         "Superman is not an instructor within the database")

    def test_bad_TA(self):
        response = self.client.post("/courses/",
                                    {"name": "test", "instructor": "instructor", "instructorTA": "Hamburglar",
                                     "description": "example"}, follow=True)
